...
export default class extends UIElement {
  ...
  get(key) {
    ...
    this.log(`Get current value of state '${String(key)}' in ${this.localName} (value: '${v}') and track its use in effect`);
    return v;
  }
  ...
  set(key, value, update = true) {
    this.log(`Set ${update ? '' : 'default '}value of state '${String(key)}' in ${this.localName} to '${value}' and trigger dependent effects`);
